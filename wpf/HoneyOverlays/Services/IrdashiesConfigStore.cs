...
/// <summary>
/// Honey-eigene irdashies-Config (Lesen UND Schreiben), damit irdashies nicht mehr
/// installiert sein muss. Quelle: %LOCALAPPDATA%\HoneyOverlays\irdashies-config.json
...
public sealed class IrdashiesConfigStore
{
    ...
    private static string HoneyConfigPath => Path.Combine(
    ...
    private void EnsureLoaded()
    {
        ...
        // 1) Honey-eigene Config
        if (TryLoadFile(HoneyConfigPath, out var root))
        {
            _root = root;
            return;
        }
    ...
    private void Save()
    {
        ...
        try
        {
            var dir = Path.GetDirectoryName(HoneyConfigPath)!;
            Directory.CreateDirectory(dir);
            var json = _root.ToJsonString(new JsonSerializerOptions { WriteIndented = true });
            var tmp = HoneyConfigPath + ".tmp";
            File.WriteAllText(tmp, json);
            File.Move(tmp, HoneyConfigPath, overwrite: true);
        }
        catch (Exception ex)
        {
